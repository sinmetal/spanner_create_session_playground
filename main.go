...
func main() {
	log.Println("ssheater is ignite")
	...
	if err := WarmUp(ctx, client); err != nil {
		log.Fatalf("failed WarmUp. err = %+v", err)
	}
	...
	log.Println("ssheater is done")
}

func startSpan(ctx context.Context, name string) (context.Context, *trace.Span) {
	return trace.StartSpan(ctx, fmt.Sprintf("/ssheater/%s", name))
}
